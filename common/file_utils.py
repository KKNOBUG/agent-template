...
def sanitize_upload_filename(raw: str, max_length: int = 200) -> str:
    """清洗上传文件名: 仅保留最后一段路径名, 拒绝一切路径穿越形态。

    上传文件名由客户端在 multipart 的 filename 字段自行填写, 可能携带
    路径分隔符或 ``../``。本函数统一分隔符后只取 basename, 并拒绝空名、
    纯点号、首尾带点空格、含控制字符、超长等非法形态; 调用方落盘后还应
    以 ``Path.resolve().is_relative_to(目标目录)`` 做纵深兜底校验。

    :param raw: 客户端传入的原始文件名
    :param max_length: 文件名长度上限（字符数）
    :return: 清洗后的纯文件名
    :raises ParameterException: 文件名为空或形态非法
    """
    if not raw:
        raise ParameterException(message="未提供文件名")
    # 统一 Windows/POSIX 分隔符后仅取最后一段
...
async def read_upload_with_cap(file, max_bytes: int) -> bytes:
    """流式读取上传文件, 超过字节上限即抛参数异常（不再全量 ``await file.read()``）。

    中间件已按 Content-Length 前置拒绝超限请求; 本函数防御请求头伪造 /
    分块传输（chunked）等绕过手段, 同时把单次读取控制为 4MB 分块。

    :param file: FastAPI ``UploadFile`` 对象
    :param max_bytes: 允许的最大字节数
    :return: 文件全部字节
    :raises ParameterException: 文件实际大小超过上限
    """
    chunks = []
    total = 0
    while True:
