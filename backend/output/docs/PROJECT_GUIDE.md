...
用户提问
  │
  ▼
chat.py：鉴权 → 创建/加载 Conversation → 读取历史 Message
  │
  ▼
chain.rag_stream()
  ├─ (可选) 无关问题短路 → 固定话术回复
...
| 库 | 作用 |
|----|------|
| **sqlalchemy[asyncio]** | 异步 ORM，管理 User/KB/Conversation 等 |
| **aiomysql** | MySQL 异步驱动 |
...
├── config.py            # 环境变量、Prompt 模板、分块/检索参数
├── database.py          # 异步 SQLAlchemy 引擎与会话
├── auth.py              # JWT、密码哈希、get_current_user 依赖
├── models.py            # ORM 模型 + Pydantic 请求/响应模型
├── api/
│   ├── auth.py          # 注册/登录/me；新用户复制 admin 配置
...
└── rag/
    ...
    ├── embeddings.py    # OpenAI 兼容 Embedding 封装（硅基流动等）
    ├── loader.py        # PDF 加载与分块工具
    ├── chroma_store.py  # Chroma 本地向量存储：写入、检索、删除
```
...
```
User ─┬─ Conversation ─── Message (多轮对话)
...
4. **异步数据库**：SQLAlchemy async，适合 I/O 密集场景。
...
| P1 | 会话结束生成摘要存 `Conversation.summary` | `chat.py` + `models.py` |
| P2 | 引入 LangGraph / 自研 MemoryStore，向量检索历史重要片段 | 新模块 `app/memory/` |
...
改动核心：`app/rag/chain.py`、新增 `app/rag/retriever.py`。
...
| 阶段 | 方案 |
|------|------|
| P0 | 定义 `Tool` 基类 + OpenAI function calling | 新模块 `app/agent/tools.py` |
| P1 | `chat.py` 改为 Agent 编排器，支持多轮 tool call |
...
改动点：新模块 `app/mcp/`，与 `app/agent/` 共用 Tool 抽象层。
...
| 现象 | 可能原因 | 排查 |
|------|----------|------|
| `LLM_API_KEY 未设置` | `.env` 未配置或未重启 | 检查 `app/config.py` 加载 |
