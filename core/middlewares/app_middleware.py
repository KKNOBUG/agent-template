...
def is_longtext_response(response: Response) -> bool:
    ...
    :param response: 响应对象。
    :returns: Content-Length 大于 102400 时返回 True。
    """
...
async def logging_middleware(request: Request, call_next):
    """记录请求与响应摘要、耗时；对上传/下载/大响应体做特殊处理。

    说明: 本项目未启用用户体系与审计表, 因此审计日志仅输出到 LOGGER, 不落库。
    ...
    is_multipart: bool = "multipart/form-data" in request.headers.get("content-type", "").lower()

    # 服务端上传大小限制: 读取请求体之前前置拒绝——前端的大小校验可被直接
    # 构造请求绕过, 无此防线时超大请求体会被全量缓冲、打爆进程内存
    ...
    if is_multipart:
        # 上传请求不做全量缓冲、也不预解析表单: 否则超大文件体会在中间件与
        # 端点各被完整读取一次, 内存与 IO 双倍开销; 审计日志仅记录元数据
        request_body = "<MULTIPART UPLOAD>"
    else:
        ...
        request_body = original_request_body.decode("utf-8", errors="ignore")
