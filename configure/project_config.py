...
class ProjectConfig(BaseSettings):
    ...
    APP_DESCRIPTION: str = """
    - 企业级 RAG（检索增强生成）问答系统 - jiayueyang 个人开发环境
    - 基于 FastAPI 构建，集成 PDF 智能解析、向量检索与流式问答能力
    """
    APP_DOCS_URL: str = "/docs"
    APP_REDOC_URL: str = "/redoc"
    # 注意: 本项目前端 ApiPage 依赖默认 OpenAPI 地址, 保持 /openapi.json
    ...
    # ==================== RAG 分块 / 检索配置 ====================
    CHUNK_SIZE: int = Field(default=1200, description="分块大小(token 估算基准)，建议 200-2000")
    ...
    LLM_BASE_URL: str = Field(
        ...
        description="LLM API Base URL (OpenAI 兼容, 环境变量兼容旧名 LLM_API_BASE)",
    )
    ...
    # 数据库配置（仅支持 MySQL; 连接项由 .env 注入, 连接池与 DATABASE_URL
    # 在 assemble_connection_urls 中装配; 未配置必填项时启动即报错）
    ...
    # Redis 配置（Celery 的 broker / result backend / redbeat 均基于 Redis 分库）
    REDIS_URL: str = ""
    ...
    def validate_env_and_assemble_urls(self) -> Self:
        ...
        # 数据库强制 MySQL: 任一必填连接项缺失即快速失败, 避免运行期才暴露配置问题
        for field_name in ("DATABASE_HOST", "DATABASE_USERNAME", "DATABASE_PASSWORD", "DATABASE_NAME"):
    ...
    def assemble_connection_urls(self) -> Self:
        # MySQL（asyncmy 异步驱动）: utf8mb4 + 东八区;
        # 连接池按进程计（gunicorn 各 worker 与 Celery worker 各自持有独立池）
        self.DATABASE_URL = (
            f"mysql://{quote_plus(self.DATABASE_USERNAME)}:{quote_plus(self.DATABASE_PASSWORD)}"
        ...
        self.DATABASE_CONNECTIONS = {
            "default": {
                ...
                "credentials": {
                    ...
                    "minsize": 2,             # 连接池下限
                    "maxsize": 10,            # 连接池上限
                    "pool_recycle": 3600,     # 连接回收秒数（防 MySQL wait_timeout 断连）
                    "charset": "utf8mb4",
                    "echo": False,
                    "autocommit": True,
                },
...
# Aerich CLI 入口（pyproject.toml 的 [tool.aerich] 指向本变量）:
# 供手工执行 aerich init / init-db / migrate / upgrade 等命令使用;
# 应用启动时的代码式迁移走 register_database(), 两处配置须保持一致。
TORTOISE_ORM = {
